...
2. Открой индикатор в GNOME top bar: `•••`.
3. Нажми **Add Account…**.
4. CBL откроет страницу подтверждения OpenAI в браузере.
5. В popup появится одноразовый **Code**.
6. Нажми на строку **Code**, чтобы скопировать код, и вставь его в браузере.
7. После подтверждения в браузере нажми **I confirmed login** в popup.
8. CBL сразу сделает refresh, и карточка аккаунта появится в списке.
